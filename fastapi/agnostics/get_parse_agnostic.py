...
def get_im_parse_agnostic(im_parse, pose_data, w=768, h=1024):
    ...
    parse_lower = ((label_array == 9).astype(np.float32) +
                   ...
                   (label_array == 16).astype(np.float32) +
                   (label_array == 17).astype(np.float32) +
                   (label_array == 18).astype(np.float32) +
                   (label_array == 19).astype(np.float32))
    ...
    for parse_id, pose_ids in [(14, [2, 5, 6, 7]), (15, [5, 2, 3, 4])]:
        ...
        #from fastapi.agnostics.utils import draw_img_GRAY
        #draw_img_GRAY(agnostic,'1')
...
def get_parse_agnostic(model_fname):
    data_path = '../data/test'
    output_path = "../data/test/image-parse-agnostic-v3.2"
